...
def create_tf_example(
    ...
    class_names = build_class_names(categories)
    ...
    partial_flags = [int(bool(flag)) for flag in is_partial]
    ...
    feature = {
        ...
        "image/object/is_partial": tf.train.Feature(
            int64_list=tf.train.Int64List(value=partial_flags)
        ),
...
def build_record(
    ...
    writer = tf.io.TFRecordWriter(str(target))
    ...
    for i in indices:
        ...
        for bbox in sample["plant_bboxes"]:
            ...
            partial_flags.append(1 if is_partial else 0)

            raw_boxes.append(phenobench_bbox_to_xyxy(bbox))
    ...
    return {
        ...
        "hard_negatives": negatives if not skip_negatives else 0,
    }
